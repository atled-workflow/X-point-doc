...
* 制限事項
  ...
  * 各フィールドの入力値の書式チェックはできません。各フィールドの書式に合わせてリクエスト内容の書類データを指定する必要があります。
    ...
    以下の例のように「eFormMakerの属性/書式タブの設定値に合致しない値を入力する」「フィールドタイプに合致しない値を入力する」といった場合に、入力内容の校正やクリアが行われることを指します。

    ブラウザ上で値を入力した場合は書式チェックが動作して入力内容の校正やクリアが行われますが、APIによる書類操作の場合は書式チェックが動作せずリクエストに指定した入力内容そのものが登録されます。

    **例）**

    * 入力可能最小値、入力可能最大値が設定された数値または整数フィールドに範囲外の数値を入力する
    * 日付書式が設定された文字フィールドに設定した書式以外の日付を入力する
    * 数値フィールドに数値以外の文字列を入力する
...
### 3.2. 書類表示API／新規書類・関連書類作成API

* 制限事項
  
  APIで書類を表示した場合、閉じるナビボタンで書類を閉じることができません。<br>書類を閉じる場合はブラウザのウィンドウやタブの×ボタンから閉じてください。
